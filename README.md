...
| 垂木拾い3Dビューア | `build_taruki_viewer.py` | 垂木配置 + 3Dビューア + 積算 |
...
│   ├── build_taruki_viewer.py         # 垂木拾い3Dビューア
...
IFCファイルから屋根勾配パラメータを自動検出し、垂木の配置位置と本数・長さを積算。

- 基本ピッチ: 455mm間隔で自動配置
- ログ壁脇: 壁のエッジ位置に必ず配置
- 天窓・煙突: 開口部の両脇にダブル配置（45mmオフセット）
- 切妻屋根の両面（前面・背面）に対称配置
- 3Dビューアで配置位置を色分け表示（理由別: オレンジ=基本ピッチ、緑=ログ壁脇、赤=天窓脇）
- JSON集計出力で部材一覧Excel統合にも対応
